...
export async function sendDemoEmailGmail(data: EmailData) {
  ...
  try {
    const result = await transporter.sendMail({
      from: `"FrameFlow Demo" <${process.env.GMAIL_USER}>`,
      to: email,
...
export async function sendNotificationEmailGmail(data: EmailData) {
  const { name, email, company, useCase } = data;
  ...
  try {
    const result = await transporter.sendMail({
      from: `"FrameFlow Notifications" <${process.env.GMAIL_USER}>`,
